...
func MySQL84(cfg *conf.Config) error {
	// 下载路径
	destDir := "/tmp"
	// 解压包名
	fileName := "mysql84.tar.xz"
	...
	if err == nil { // 如果文件存在
		// 对比 MD5值
		if err := utils.DiffMD5(filePath, conf.Mysql84DownloadMD5); err != nil {
			err := utils.Download(conf.Mysql84DownloadURL, destDir, fileName, conf.Mysql84DownloadMD5)
			if err != nil {
