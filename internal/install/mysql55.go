...
func MySQL55(cfg *conf.Config) error {
	// 下载路径
	destDir := "/tmp"
	// 解压包名
	fileName := "mysql55.tar.gz"
	...
	if err == nil { // 如果文件存在
		// 对比 MD5值
		if err := utils.DiffMD5(filePath, conf.Mysql55DownloadMD5); err != nil {
			err := utils.Download(conf.Mysql55DownloadURL, destDir, fileName, conf.Mysql55DownloadMD5)
			if err != nil {
	...
	// 生成配置文件
	if err := utils.ConfFile(cfg, mysqlUser, "templates/my55.cnf.tmpl"); err != nil {
		return err
	}
	logs.Info("配置文件已生成")
	...
	// 初始化
	logs.Info("正在初始化···")
	if err := utils.Initialize56(cfg, mysqlUser); err != nil {
		return err
	}
	...
	// 配置启动脚本并启动
	if err := utils.StartFile(cfg, "templates/mysql57.service.tmpl"); err != nil {
		return err
	}
	// 快捷登录
