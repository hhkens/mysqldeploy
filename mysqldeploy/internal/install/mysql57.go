...
func MySQL57(cfg *conf.Config) error {
	// 下载路径
	destDir := "/tmp"
	// 解压包名
	fileName := "mysql57.tar.gz"
	...
	if err == nil { // 如果文件存在
		// 对比 MD5值
		if err := utils.DiffMD5(filePath, conf.Mysql57DownloadMD5); err != nil {
			err := utils.Download(conf.Mysql57DownloadURL, destDir, fileName, conf.Mysql57DownloadMD5)
			if err != nil {
	...
	// 生成配置文件
	if err := utils.ConfFile(cfg, mysqlUser, "templates/my57.cnf.tmpl"); err != nil {
		return err
	}
	logs.Info("配置文件已生成")
